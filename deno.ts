...
async function requestAzure(request: Request, body: any, path: string, authKey?: string) {
...
async function handleDirect(request: Request, path: string) {
  ...
  const response: Response = await requestAzure(request, body, path, key);
  if (body?.stream != true){
    return response
  } 
  if (response.body) {
...
async function handleEmbedding(request: Request, path: string) {
  ...
  if (typeof input === "string") {
      return await requestAzure(request, body, path, key);
  } else if (Array.isArray(input)) {
      const resps = pooledMap(3,
          input, (x: any) => {
              return requestAzure(request, { ...body, input: x }, path, key);
          });
      const retbody = {
          object: "list",
          data: [] as any[],
